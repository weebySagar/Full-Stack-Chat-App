...
const PopupMenu = ({ groupId, userId, closeModal }) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div
          className={`absolute right-0 mt-1 w-40 bg-white border border-gray-200 rounded shadow-lg z-10 popup-menu`}
        >
          {/* Menu items */}
          <button
            onClick={handleRemoveUser}
            className="block w-full px-4 py-2 text-red-500 hover:bg-gray-100 text-xs text-start"
          >
            Remove
          </button>
          <button
            onClick={handleMakeUserAdmin}
            className="block px-4 py-2 w-full text-gray-800 hover:bg-gray-100 text-xs text-start"
          >
            Make admin
          </button>
        </div>
